...
import { Check, ChevronRight, Dices, LoaderCircle, Pencil, Plus, Trash2, X } from "lucide-react";
...
export function TeamTermsPreview({ teamId, terms }: { teamId: string; terms: TeamTerm[] }) {
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between gap-3">
          <span className="flex items-center gap-2">
            <Dices className="size-5 text-primary dark:text-violet-300" aria-hidden="true" />
            Bingo terms
          </span>
      ...
      <CardContent className="flex flex-1 flex-col gap-4">
        {shownTerms.length ? (
          <ul className="grid gap-2" aria-label="Bingo term preview">
            {shownTerms.map((term) => (
              <li className="truncate rounded-lg bg-muted/45 p-3 font-medium" key={term.id}>
                {term.label}
              </li>
            ))}
          </ul>
