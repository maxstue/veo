      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between gap-3">
          <span className="flex items-center gap-2">
            <Settings2 className="size-5 text-primary dark:text-violet-300" aria-hidden="true" />
            Bingo rules
          </span>
  ...
  return (
    <Card className="flex h-full flex-col">
      <CardHeader>
        ...
        <CardDescription>
          Changes apply to newly shuffled cards. Existing cards keep their rules.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col gap-4">
        <label className="grid gap-1.5 text-sm font-medium">
          ...
          <select
            ...
            disabled={pending}
            onChange={(event) =>
              setBoardSize(Number(event.target.value) as BingoRules["boardSize"])
            }
            value={boardSize}
          >
        ...
        <form className="mt-auto flex gap-2" onSubmit={(event) => void savePreset(event)}>
          ...
          <input
            className="h-10 min-w-0 flex-1 rounded-lg border bg-background px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30"
            disabled={pending}
            ...
            {presets.map((preset) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{getRulesSummary(preset)}</p>
                </div>
                <div className="flex shrink-0">
                  <Button
                    ...
                    className={
                      preset.id === defaultPresetId
                        ? "cursor-default bg-primary/15 text-primary disabled:opacity-100 dark:bg-violet-300/20 dark:text-violet-200"
...
function getRulesSummary(rules: BingoRulesConfig) {
  const patterns = [
    rules.horizontal && "rows",
    rules.vertical && "columns",
    rules.diagonal && "diagonals",
  ]
    .filter(Boolean)
    .join(", ");
