...
describe('bingo detection', () => {
  ...
  test('keeps an achieved bingo in the score after the card changes', () => {
    ...
    expect(getBingoCompletionTime(completedAt, false, new Date('2026-08-06T09:00:00.000Z'))).toBe(completedAt);
  });
});
...
describe('bingo term selection', () => {
  ...
  test('requires enough terms for a full card', () => {
    expect(() => selectBingoTerms(Array.from({ length: 24 }), () => 0)).toThrow('At least 25 bingo terms are required');
  });
